...
x = kfcframe.Length
y = kfcframe.Likes
kfcframe.plot.scatter(x = 'Length', y = 'Likes')
plt.show()


kfcframe2 = kfcframe.groupby('Day').agg({'Likes': 'sum','Haha': 'sum','Angry': 'sum'}).reset_index()

plt.plot(kfcframe2['Day'], kfcframe2['Likes'])
plt.plot(kfcframe2['Day'], kfcframe2['Haha'])
plt.plot(kfcframe2['Day'], kfcframe2['Angry'])
